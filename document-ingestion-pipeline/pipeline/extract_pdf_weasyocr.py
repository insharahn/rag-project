...
Two OCR engines, chosen once per document (not per page):
  - Tesseract for English and Korean documents (fast, validated at
    ~90%+ confidence across the corpus).
  - EasyOCR for Urdu documents, since Tesseract's accuracy on Urdu's
    ...
    EasyOCR is not a magic fix -- no current open-source OCR engine
    handles Nastaliq well -- but it's a measurable, testable alternative.

Routing is decided from a single cheap, low-resolution sample (one
Tesseract call on the first page needing OCR), not by running full OCR
on every page with both engines -- that would mean paying for both
engines' full cost on every Urdu page, which is wasteful and was the
mistake in an earlier version of this file.

Known limitation: this assumes one dominant OCR language per document.
A scanned PDF that's genuinely mixed-script page-to-page would have the
whole document routed by whatever the first OCR page looks like. Not
true of any document in the current corpus, but worth a note if that
ever changes.
"""
